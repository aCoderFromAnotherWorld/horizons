import { DOMAIN_WEIGHTS } from './domains.js';
 ...
 * capped at 2.0×).
 *
 * Formula: combined = Σ(rawScore[domain] × weight[domain]) × redFlagMultiplier
 *
 * This matches the stable-v0 calibration. The COMBINED_THRESHOLDS are designed
 * for this formula where the theoretical max (all domains at max raw points) is
 * 100×0.40 + 70×0.30 + 40×0.15 + 30×0.15 = 71.5.
 *
 ...
 * @returns {number} Combined score rounded to 1 decimal place.
 */
export function calculateCombinedScore(domainRawScores, activeRedFlags = []) {
  ...
  return Math.round(combined * multiplier * 10) / 10;
}
