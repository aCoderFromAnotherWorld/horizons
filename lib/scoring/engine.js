import { DOMAIN_WEIGHTS, DOMAIN_MAX_POINTS } from './domains.js';
 ...
 * @returns {number} Combined score (REAL).
 */
export function calculateCombinedScore(domainRawScores, activeRedFlags = []) {
  ...
  for (const [domain, weight] of Object.entries(DOMAIN_WEIGHTS)) {
    ...
    const maxPts = DOMAIN_MAX_POINTS[domain];
    // Normalise raw score to 0–100 scale, then apply domain weight.
    const normalised = maxPts > 0 ? (raw / maxPts) * 100 : 0;
    weightedSum += normalised * weight;
  }

  // Stack multipliers multiplicatively, cap at MAX_MULTIPLIER.
  let multiplier = 1.0;
