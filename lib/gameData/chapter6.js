...
function isCorrectOption(option) {
  return option.correct === true || option.scorePoints === 0;
}

function shuffleAnswerOptions(task, seed) {
  if (!Array.isArray(task.options) || task.options.length < 2) return task;
  if (task.mechanic !== 'grid_select' && task.mechanic !== 'scenario_choice') return task;

  const rng = seededRng(`${seed}:${task.key}:options`);
  let options = shuffleWithRng(task.options, rng);

  // Avoid making the first answer a reliable hint in sampled finale tasks.
  if (isCorrectOption(options[0])) {
    const swapCandidates = options
      .map((option, index) => ({ option, index }))
      .filter(({ option, index }) => index > 0 && !isCorrectOption(option));
    const targetIndex = swapCandidates.length > 0
      ? swapCandidates[Math.floor(rng() * swapCandidates.length)].index
      : 1 + Math.floor(rng() * (options.length - 1));
...
export function sampleTasks(sessionId) {
  ...
  return shuffledPool
    .slice(0, SAMPLE_SIZE)
    .map((task) => shuffleAnswerOptions(task, sessionId));
}
