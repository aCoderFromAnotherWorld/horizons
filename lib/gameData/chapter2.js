...
const subjects = ["child", "adult", "dog"];

function facePath(emotion, subject = "child") {
  return `/assets/emotions/${subject}_${emotion}.png`;
}
...
export const faceCards = emotions.flatMap((emotion) =>
  subjects.map((subjectType) => ({
    ...
    emotion,
    subjectType,
    imagePath: facePath(emotion, subjectType),
...
function buildOptions(emotion, intensity, variantSeed = 0) {
  ...
  return [
    facePath(emotion, targetSubject),
    facePath("neutral", neutralSubject),
