...
CREATE TABLE IF NOT EXISTS sessions (
  ...
  camera_enabled INTEGER DEFAULT 0,   -- 0 or 1, caregiver consent gated
...
CREATE TABLE IF NOT EXISTS camera_frames (
  ...
  face_landmarks    TEXT, -- JSON: MediaPipe face landmarks or equivalent
  iris_landmarks    TEXT, -- JSON: iris landmarks when available
  pose_landmarks    TEXT, -- JSON: pose landmarks when available
  gaze_direction    TEXT, -- JSON: normalized gaze vector when available
  blink_rate        REAL,
  head_pose         TEXT, -- JSON: pitch/yaw/roll when available
  expression_scores TEXT, -- JSON: expression probability map
  extra_data        TEXT  -- JSON: capture/model metadata
);
...
CREATE INDEX IF NOT EXISTS idx_camera_frames_session ON camera_frames(session_id);
CREATE INDEX IF NOT EXISTS idx_camera_frames_task ON camera_frames(session_id, task_key);
