...
if (!process.env.RESEND_FROM) {
  console.warn('[email] RESEND_FROM not set — emails will use fallback sender address');
}
...
export async function sendContactNotification({ name, email, role, message }) {
  ...
  const from = process.env.RESEND_FROM ?? 'noreply@horizons-asd.vercel.app';
  const htmlBody = `
  ...
  `;
  const textBody = [
    'New Contact Form Submission — Horizons',
    '',
